...
//connecte le robot à l'ordinateur et retourne un booléen pour vérifier l'état de la connexion
bool MyRobot::doConnect2()
{
        socket = new QTcpSocket(this); // socket creation
        connect(socket, SIGNAL(connected()),this, SLOT(connected()));
        connect(socket, SIGNAL(disconnected()),this, SLOT(disconnected()));
...
//permet de se déconnecter du robot
...
float MyRobot::getBatteryPercent()
{
    ...
    qDebug() <<"battery  level= "<<(adc1*100/185);
    batteryLevel=adc1;
    return adc1;
}
...
int MyRobot::getSpeddL()
{
    return speedL;
}

int MyRobot::getSpeedR()
{
    return speedR ;
}
